...
### **공변성(covariant)**
***
위의 예시에서 List는 넘겨 받은 타입 파라미터의 상속 관계를 그대로 따라간다는 걸 코드를 통해서 봤다.  
```kotlin
val stringList = listOf("a")  
val anyList :List<Any> = stringList
println(anyList)  // [a] (가능)
```
List와 같이 **타입 파라미터의 상속 관계를 제네릭 클래스에서도 그대로 가져갈 때**, **공변성을 가진다**고 한다.  
...
### **불공변성(invariant)**
***
위의 예시에서 MutableList는 넘겨 받은 타입 파라미터의 상속 관계를 따르지 않았다.  
...
타입 파라미터 앞에 아무 키워드도 적혀있지 않다면 불공변성을 가진다.  
...
### 솔직히 어떤 의미 인지는 잘 알겠는데, 이러한 변성이 왜 필요한가 잘 와닿지 않는다.  

예제를 보면서 왜 필요한지 알아보자.  
아래는 Collection을 Comparator내용에 맞게 정렬한 List를 반환해주는 sortedWith 확장 함수의 선언부다.  
```kotlin
public fun <T> Iterable<T>.sortedWith(comparator: Comparator<in T>): List<T> { 
	//클래스가 아닌 파라미터에 반공변성 키워드인 in이 적혀있다. (comparator: Comparator<in T>)
	//이런 경우를 "사용 지점 변성" 이라 한다. 
	//클래스 전체에 해당하는게 아닌 해당 함수 파라미터에서만 반공변성이 적용된다.)
...
Comparator인터페이스의 compare함수에 어떤 프로퍼티 값으로 객체간 대소를 비교하면 되는지 명시해서 override 해주면 된다.   
(단일 추상 클래스라서 SAM 변환이 가능해 람다만으로 정의가 가능하다.)  
```kotlin
...
Comparator\<Hyundai> 타입 파라미터 자리에 Comparator\<Car>타입의 변수를 집어 넣을 수 있어진다.  

고로, 상위 타입에 정의된 내용(프로퍼티 등)으로 하위 타입에도 똑같은 로직을 적용할 수 있게 하려면  
반공변성이 방법이 될 수 있다.  

</br>

## 아무튼 그래서 List는 공변성을 가지고, MutableList는 불공변성을 가지는 이유는 뭘까?
***
...
```kotlin
val stringMutableList = mutableListOf("n")  
val anyMutableList :MutableList<Any> = stringMutableList  
anyMutableList.add(26)
```
MutableList\<String> 타입으로 정의된 stringMutableList타입에 Int타입인 26이 들어가는 상황이 생긴다.  
**즉, 타입 안정성이 사라진다.**  
그래서 코틀린 컴파일러는 변성 규칙을 준수해서 제네릭 클래스를 정의하게 강제한다. 
...
3. 제네릭 클래스 내부에서 타입 파라미터가 **out위치, in위치 둘 다** 쓰이는 경우는 **공변성, 반공변성을 가질 수 없다.**
...
#### 공변성을 가지려면 out위치에만 타입 파라미터가 존재해야 한다.  

out 위치뿐만 아니라 in위치에서도 타입 파라미터가 존재하는데 공변성을 가지면
위에 보여준 MutableList처럼 타입 안정성이 깨지게 된다.

#### 반공변성을 가지려면 in위치에만 타입 파라미터가 존재해야 한다.  

in위치뿐만 아니라 out 위치에서도  타입 파라미터가 존재하는데 반공변성을 가지면
